...
def crear_pedidos(conexion):
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM pedido WHERE idpedido = %s", (idcliente,))
        if cursor.fetchone()[0] == 0:
            print(f"No existe un pedido con ID {idcliente}.")
            return
...
def eliminar_pedido(conexion):
    try:
        ...
        id = int(id)
        consulta = """DELETE FROM categoria WHERE idpedido = %s"""
